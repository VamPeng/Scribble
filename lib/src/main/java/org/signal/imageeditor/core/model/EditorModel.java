...
public final class EditorModel implements Parcelable, RendererContext.Ready {
    ...
    public EditorModel(@NonNull EditingPurpose editingPurpose,
                       float fixedRatio,
                       @NonNull EditorElementHierarchy editorElementHierarchy) {
    ...
    public static EditorModel createForAvatarCapture(@ColorInt int blackoutColor) {
        EditorModel editorModel = new EditorModel(EditingPurpose.AVATAR_CAPTURE,
                                                  1,
                                                  EditorElementHierarchy.createForCircleEditing(blackoutColor));
        editorModel.setCropAspectLock(true);
        return editorModel;
    }

    public static EditorModel createForAvatarEdit(@ColorInt int blackoutColor) {
        EditorModel editorModel = new EditorModel(EditingPurpose.AVATAR_EDIT,
                                                  1,
                                                  EditorElementHierarchy.createForCircleEditing(blackoutColor));
        editorModel.setCropAspectLock(true);
        return editorModel;
    ...
    public static EditorModel createForWallpaperEditing(float fixedRatio, @ColorInt int blackoutColor) {
        EditorModel editorModel = new EditorModel(EditingPurpose.WALLPAPER,
                                                  fixedRatio,
                                                  EditorElementHierarchy.createForPinchAndPanCropping(blackoutColor));
        editorModel.setCropAspectLock(true);
        return editorModel;
    }
    ...
    public EditorElement findElementAtPoint(@NonNull PointF point,
    ...
    private boolean findElement(@NonNull EditorElement element,
                                @NonNull Matrix viewMatrix,
                                @NonNull Matrix outInverseModelMatrix) {
    ...
    public void clearUndoStack() {
        ...
        while (stacks.canUndo(root)) {
            ...
            Log.d("EditorModel", "pop element " + oldRootElement.hashCode());
    ...
    private static void restoreStateWithAnimations(@NonNull EditorElement fromRootElement,
     ...
     */
    private boolean tryToFixTranslationOutOfBounds(@NonNull EditorElement element,
                                                   @NonNull Matrix lastKnownGoodPosition) {
    ...
    public @NonNull Bitmap render(@NonNull Context context,
                                  @NonNull RendererContext.TypefaceProvider typefaceProvider) {
    ...
    public @NonNull Bitmap render(@NonNull Context context,
        ...
        try {
            ...
            RendererContext rendererContext = new RendererContext(context,
                                                                  canvas,
    ...
    public Matrix getInverseCropPosition() {
        ...
        matrix.set(findRelativeMatrix(editorElementHierarchy.getMainImage(),
                                      editorElementHierarchy.getCropEditorElement()));
