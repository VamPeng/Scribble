package com.demo.scribbledemo.renderers;
...
public final class UriGlideRenderer implements SelectableRenderer {
    ...
    public UriGlideRenderer(@NonNull Uri imageUri,
                            boolean decryptable,
                            int maxWidth,
                            int maxHeight,
                            float blurRadius,
                            @Nullable RequestListener<Bitmap> bitmapRequestListener) {
...
//    this.decryptable           = decryptable;
    ...
    public void render(@NonNull RendererContext rendererContext) {
        ...
        if (bitmap != null) {
            ...
            rendererContext.canvas.drawBitmap(bitmap,
                                              0,
                                              0,
    ...
    private void setBitmap(@NonNull RendererContext rendererContext, @Nullable Bitmap bitmap) {
        ...
        if (bitmap != null) {
            ...
            rendererContext.rendererReady.onReady(UriGlideRenderer.this,
                                                  cropMatrix(bitmap),
    ...
    private static @NonNull Bitmap blur(Bitmap bitmap, Context context, float blurRadius) {
        Point previewSize = scaleKeepingAspectRatio(new Point(bitmap.getWidth(), bitmap.getHeight()),
                                                    PREVIEW_DIMENSION_LIMIT);
        Point blurSize = scaleKeepingAspectRatio(new Point(previewSize.x / 2, previewSize.y / 2),
                                                 MAX_BLUR_DIMENSION);
        Bitmap small = createScaledBitmap(bitmap, blurSize.x, blurSize.y);

        Log.d(TAG,
              "Bitmap: " + bitmap.getWidth() + "x" + bitmap.getHeight() + ", Blur: " + blurSize.x + "x" + blurSize.y);
